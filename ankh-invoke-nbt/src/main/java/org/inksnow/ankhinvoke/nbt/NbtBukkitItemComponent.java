...
public class NbtBukkitItemComponent implements NbtComponentLike {
  private static final Logger logger = Logger.getLogger(AnkhInvokeNbt.ANKH_INVOKE_NBT_PACKAGE);
  private static final Set<String> HANDLED_TAGS = Collections.unmodifiableSet(new LinkedHashSet<>(RefCraftMetaItem.HANDLED_TAGS));
  ...
  public Nbt<?> get(String key) {
    if (HANDLED_TAGS.contains(key)) {
      logger.warning("key " + key + "isn't support for direct access");
      return null;
    } else {
